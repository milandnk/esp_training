...
{
    //file open or create if not exist
    ...
    //REMOVE FILE INTO SD CARD
    if (remove(file_hello) == 0)
    {
    ...
    ESP_LOGE(TAG, "after remove 1 file aftersize");

    //AFTER REMOVING FILE PRINT INFO
    sdmmc_card_print_info(stdout, card);
