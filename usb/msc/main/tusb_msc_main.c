...
static int console_write(int argc, char **argv)
{
    ...
    const char *filename = BASE_PATH "/file.txt";
    FILE *fd = fopen(filename, "r");
    if (!fd) {
        ESP_LOGW(TAG, "file.txt doesn't exist yet, creating");
        fd = fopen(filename, "w");
        fprintf(fd, "hello milan\n");
        fclose(fd);
    }
    return 0;
}
...
static int console_size(int argc, char **argv)
{
    ...
    uint32_t total_size_kb = ((uint64_t) sec_count) * sec_size / 1024;
    printf("Storage Capacity: %lu kB\n", total_size_kb); 
    return 0;
}
